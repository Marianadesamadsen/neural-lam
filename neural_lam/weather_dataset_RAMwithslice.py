...
class WeatherDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.precompute_in_memory:
            logger.info(f"Loading full {self.split} split into memory")

            self.da_state = self.da_state.load()

            if self.da_forcing is not None:
                self.da_forcing = self.da_forcing.load()

            logger.info(f"Loaded full {self.split} split into memory")
    ...
    def _build_item_dataarrays(self, idx):
        ...
        # If not already cached in memory, load only this sliced sample.
